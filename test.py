...
class FeatureBrokerTest(TestCase):
    def test_exception_raised_for_unprovisioned_feature(self):
        with self.assertRaises(MissingFeatureException) as context_manager:
            FeatureBroker().get_feature('foo')
        self.assertEqual(context_manager.exception.message, "Feature 'foo' has not been provisioned")

    def test_feature_returned_when_provisioned(self):
